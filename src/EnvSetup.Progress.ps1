...
function New-TaskResult {
    ...
    return [pscustomobject]@{
        taskId = $TaskId; name = $Name; status = $Status; durationMilliseconds = [Math]::Round($Duration.TotalMilliseconds)
        phase = $Phase; errorCode = $ErrorCode; message = $Message
    }
}
...
function Show-SetupExecutionSummary {
    ...
    $summary = [pscustomobject]@{
        completed = [int]($counts['completed']); alreadyConfigured = [int]($counts['already-configured'])
        planned = [int]($counts['planned']); configured = [int]($counts['configured']); missing = [int]($counts['missing'])
        failed = [int]($counts['failed']); durationMilliseconds = [Math]::Round($Duration.TotalMilliseconds)
        rebootRequired = $rebootRequired; logPath = $Context.Paths.LogPath
    }

    Write-SetupMessage -Message '' -Level Muted
    ...
    if ($Failed -or $rebootRequired) {
        Write-SetupMessage -Message '  Continue with: .\setup.ps1 -Resume' -Level Warning
    }
    return $summary
}
