...
function Get-EnvSetupDoctorChecks {
    ...
    try {
        ...
        $checks.Add((New-DoctorCheck -Name 'Free disk space' -Status $(if ($freeGb -ge 15) { 'pass' } elseif ($freeGb -ge 5) { 'warning' } else { 'fail' }) -Details ("{0} GB available on {1}:" -f $freeGb, $driveName) -Code 'ENVSETUP-DOCTOR-DISK'))
    }
    ...
    try {
        ...
        $checks.Add((New-DoctorCheck -Name 'WSL command' -Status $(if ($wslStatus.ExitCode -eq 0) { 'pass' } else { 'warning' }) -Details $(if ([string]::IsNullOrWhiteSpace($wslStatus.Text)) { "Exit code $($wslStatus.ExitCode)." } else { ($wslStatus.Text -replace '\s+', ' ').Trim() }) -Code 'ENVSETUP-DOCTOR-WSL'))
    }
    ...
    if (-not $SkipNetwork) {
        foreach ($endpoint in @(
            ...
            try {
                ...
                $checks.Add((New-DoctorCheck -Name ("Network: {0}" -f $endpoint.Name) -Status $(if ($reachable) { 'pass' } else { 'fail' }) -Details ("{0}:443" -f $endpoint.Host) -Code ("ENVSETUP-DOCTOR-NETWORK-{0}" -f ($endpoint.Name -replace '[^A-Za-z0-9]', '').ToUpperInvariant())))
            }
...
function Invoke-EnvSetupDoctor {
    ...
    Write-SetupMessage -Message ("Doctor result: {0} passed, {1} warnings, {2} failures." -f (@($checks | Where-Object status -eq 'pass').Count), $warnings.Count, $failed.Count) -Level $(if ($failed.Count -gt 0) { 'Error' } elseif ($warnings.Count -gt 0) { 'Warning' } else { 'Success' }) -Event 'doctor-summary'
...
function Reset-EnvSetupSelections {
    ...
    if (-not (Test-Path -LiteralPath $Paths.PlanPath -PathType Leaf)) {
        Write-SetupMessage -Message 'No saved selections exist.' -Level Muted
        return
    }
...
function New-EnvSetupDiagnosticsBundle {
    ...
    if ([string]::IsNullOrWhiteSpace($Destination)) {
        $Destination = Join-Path $HOME ("env-setup-diagnostics-{0}.zip" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
    }
    ...
    try {
        ...
        if (Test-Path -LiteralPath $Paths.PlanPath -PathType Leaf) {
            $planText = Protect-DiagnosticText -Text (Get-Content -LiteralPath $Paths.PlanPath -Raw)
            [System.IO.File]::WriteAllText((Join-Path $tempRoot 'plan.redacted.json'), $planText, [System.Text.UTF8Encoding]::new($false))
        }
        if (Test-Path -LiteralPath $Paths.StatePath -PathType Leaf) {
            $stateText = Protect-DiagnosticText -Text (Get-Content -LiteralPath $Paths.StatePath -Raw)
            [System.IO.File]::WriteAllText((Join-Path $tempRoot 'state.redacted.json'), $stateText, [System.Text.UTF8Encoding]::new($false))
        }
        ...
        Compress-Archive -LiteralPath (Join-Path $tempRoot '*') -DestinationPath $Destination -Force
